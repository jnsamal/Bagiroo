...
function ContentEditor({ resource }) {
  ...
  return <div>
    ...
    <div className="space-y-3">{data.items.map(item => <div key={item.id} className="admin-panel flex flex-wrap justify-between items-center gap-4">
      <div className="min-w-0"><p className="font-medium">{item.name || item.code || item.message || item.authorName || item.caption || item.product?.title || item.variation?.product?.title || data.products.find(product => product.id === item.productId)?.title || 'Untitled'}</p><p className="text-xs text-muted mt-1">{resource === 'contacts' ? `${item.email} · ${item.subject} · ${item.status}` : resource === 'reviews' ? `${data.products.find(product => product.id === item.productId)?.title || 'Product'} · ${item.rating}/5 · ${item.isApproved ? 'Approved' : 'Pending approval'}` : item.slug || item.url || item.variation?.sku || item.product?.sku || ''}{resource === 'inventory' ? ` · On hand: ${item.quantityAvailable} · Reserved: ${item.quantityReserved} · Sellable: ${item.quantityAvailable - item.quantityReserved}` : ''}</p>{resource === 'reviews' && <><p className="mt-3 whitespace-pre-wrap text-sm">{item.comment}</p><p className="mt-2 text-xs text-muted">{new Date(item.createdAt).toLocaleString()}</p></>}{resource === 'contacts' && <><p className="text-sm mt-3 whitespace-pre-wrap">{item.message}</p>{(item.phone || item.orderNumber) && <p className="text-xs mt-2">{item.phone ? `Phone: ${item.phone}` : ''}{item.phone && item.orderNumber ? ' · ' : ''}{item.orderNumber ? `Order: ${item.orderNumber}` : ''}</p>}<p className="text-xs text-muted mt-2">{new Date(item.createdAt).toLocaleString()}</p></>}</div>
